...
mytask: 
    ...
    mov bx, [bp+6]          ; Load column number parameter 
...
initpcb:
    ...
    mov ax, [bp+10]                ; Read segment parameter
    mov [pcb+bx+18], ax            ; Save in PCB space for cs
    mov ax, [bp+8]                 ; Read offset parameter
    ...
    mov ax, [bp+6]                 ; Read parameter for subroutine
    sub si, 2                      ; Decrement thread stack pointer
    mov [si], ax                   ; Pushing param on thread stack
